...
def varen_premik(x0, y0, x1, y1, mine):
    ...
    return False if False in [False if x in range(min(x0, x1), max(x0, x1)+1) and
                                       y in range(min(y0, y1), max(y0, y1)+1) else True for x, y in mine] else True
...
def varna_pot(pot, mine):
    ...
    #!!!!!!!!!!!!!!!!!Dodaj še na dolgo za vsak slučaj

    # !!!!!!!!!!!!!!!!!Dodaj še na dolgo za vsak slučaj

    # return False if False in [  varen_premik(t1[0], t1[1], t2[0], t2[1], mine) if pot[0] not in mine else False for t1, t2 in zip(pot, pot[1:]) ] else True
